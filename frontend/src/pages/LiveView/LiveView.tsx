    ...
    try {
      ...
      ws.onerror = () => {
        ...
        notifications.show({
          ...
          message:
            "Unable to connect to live camera feed. Make sure the backend is running.",
          color: "red",
          icon: <IconCameraOff size={16} />,
        });
      };
  ...
  useEffect(() => {
    ...
    detections.forEach((detection, index) => {
      ...
      if (confidence >= 0.8) color = "#51cf66"; // green for high confidence
        ...
        color = "#ffd43b"; // yellow for medium confidence
      ...
      ctx.fillRect(
        x + 2,
        y - 18,
        (confidenceWidth - 4) * confidence,
        confidenceHeight
      );

      // Draw label if enabled
      if (showLabels) {
        const label = `${detection.class_name} ${(confidence * 100).toFixed(
          0
        )}%`;
        ctx.fillStyle = color;
        ctx.font = "14px Arial";
      ...
      if (index < 3) {
        // Only animate the 3 most recent
    ...
    <Stack gap="lg">
      ...
      {error && (
        <Alert
          ...
          color="red"
        >
          {error}. Make sure the backend server is running and the camera is
          connected.
        </Alert>
      )}
                    ...
                    <Stack align="center" gap="md">
                      ...
                      <Text c="dimmed" ta="center">
                        {isLoading
                          ...
                          : 'Click "Start Feed" to begin live view'}
                      </Text>
            ...
            <Card shadow="sm" padding="md" radius="md" withBorder>
              <Stack gap="md">
                ...
                <Group justify="space-between">
                  ...
                  <Group gap="xs">
                    ...
                    <RingProgress
                      ...
                      sections={[
                        {
                          value: Math.min(stats.fps * 10, 100),
                          color: stats.fps > 5 ? "green" : "orange",
                        },
                      ]}
                    />
                  </Group>
                ...
                <Group justify="space-between">
                  ...
                  <Text size="sm" fw={600}>
                    {stats.processing_time.toFixed(0)}ms
                  </Text>
                </Group>
                ...
                {stats.last_detection && (
                  <Group justify="space-between">
                    ...
                    <Text size="xs">
                      {dayjs(stats.last_detection).fromNow()}
                    </Text>
                  </Group>
                )}
              </Stack>
                      ...
                      >
                        <Group gap="xs">
                          <Badge
                            ...
                            size="xs"
                            color={
                              detection.confidence >= 0.8 ? "green" : "orange"
                            }
                          >
                            {detection.class_name}
                          </Badge>
